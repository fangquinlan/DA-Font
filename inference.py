...
def getCharList(root):
    '''
    getCharList
    Get list of characters from image files in directory
    Args:
        root: Directory containing character images
    Returns:
        List of character names (from filenames without extensions)
    '''
    charlist = []
...
def getMetaDict(image_root_list, content_name, cr_mapping):
    """Create metadata dictionary for fonts and characters
    Args:
        image_root_list: List of directories containing font images
        content_name: Name of content font
        cr_mapping: Character mapping dictionary
    Returns:
        Dictionary containing font metadata and character lists
    """
    meta_dict = dict() 
...
def build_meta4build_dataset(meta_path, img_path_list, content_name, cr_mapping):
    ...
    Build and save metadata JSON file for dataset construction
    Args:
        meta_path: Path to save metadata JSON
        img_path_list: List of image directories
        content_name: Name of content font
        cr_mapping: Character mapping dictionary
    '''
...
def build_testmeta4inference(target_name, target_root, content_name="kaiti_xiantu"):
    ...
    Create test metadata JSON for inference
    Args:
        target_name: Name of target font
        target_root: Root directory for test files
        content_name: Name of content font
    Returns:
        Path to saved test metadata and available character set
    '''
...
def eval_ckpt(args, cfg, avail, target_root):
    '''
    eval_ckpt
    Evaluate checkpoint by generating test images
    Args:
        args: Command line arguments
        cfg: Configuration object
        avail: Available character set
        target_root: Directory to save results
    '''
    logger = Logger.get() 
    ...
    # Load LMDB environment and setup data loading
    ...
    # Load pretrained component embeddings
...
def get_codebook_detach(component_embeddings, batch_size):
    ...
    Returns:
        Detached component embeddings prepared for batch
    """
